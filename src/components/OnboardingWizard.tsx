...
function StepPrizeCards({ cfg, onChange }: { cfg: DrawConfig; onChange: (partial: Partial<DrawConfig>) => void }) {
    ...
    return (
        <div className="onb-step-content">
            <p className="onb-step-desc">Set up your prize tiers — name and total prizes for each. Prizes are drawn in order from top to bottom. Remove all cards to skip directly to the draw screen.</p>
...
function StepStyle({ cfg, onChange }: { cfg: DrawConfig; onChange: (partial: Partial<DrawConfig>) => void }) {
    ...
    return (
        <div className="onb-step-content">
            <p className="onb-step-desc">Customize the look — choose fonts, emoji sets, accent color, and card effects.</p>
            ...
            <div className="onb-card" style={{ marginBottom: 16 }}>
                <label className="onb-label" style={{ marginBottom: 8, display: 'block' }}>😎 Emoji Set</label>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {Object.entries(EMOJI_SETS).map(([key, set]) => (
                        <button key={key} type="button" onClick={() => onChange({ emojiSet: key })}
            ...
            <div className="onb-card" style={{ marginBottom: 16 }}>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    {ACCENT_PRESETS.map(color => (
                        <button key={color} type="button" onClick={() => onChange({ accentColor: color })}
                            style={{
                    ...
                    <label style={{
                        ...
                        <input type="color" value={cfg.accentColor} onChange={e => onChange({ accentColor: e.target.value })}
            ...
            <div style={{ marginTop: 16, padding: 16, borderRadius: 12, background: `rgba(20,30,60,${cfg.cardOpacity / 100})`, backdropFilter: `blur(${cfg.cardBlur}px)`, border: `2px solid ${cfg.accentColor}40`, textAlign: 'center' }}>
                <div style={{ fontFamily: `'${cfg.fontFamily}', sans-serif`, fontSize: 20, fontWeight: 800, color: 'white', marginBottom: 4 }}>
                    {(EMOJI_SETS[cfg.emojiSet]?.emojis[0] ?? '🏆')} Preview
                </div>
                <div style={{ fontFamily: `'${cfg.fontFamily}', sans-serif`, fontSize: 14, color: 'rgba(255,255,255,0.7)' }}>
                    Font: {cfg.fontFamily} · Color: <span style={{ color: cfg.accentColor }}>■</span>
                </div>
            </div>
        </div>
    );
                ...
                <div className="onb-header">
                    ...
                    <div className="onb-steps">
                        {stepLabels.map((s, i) => {
                            const Icon = s.icon;
                            return (
                                <div key={i} className={`onb-step-pill ${i === step ? "active" : ""} ${i < step ? "done" : ""}`}>
                                    <div className="onb-step-pill-icon">{i < step ? <Check size={13} /> : <Icon size={13} />}</div>
